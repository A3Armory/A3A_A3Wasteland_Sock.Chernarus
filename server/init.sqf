...
if (_playerSavingOn || _objectSavingOn || _vehicleSavingOn || _timeSavingOn || _weatherSavingOn) then
{
	...
	[_playerSavingOn, _objectSavingOn, _vehicleSavingOn, _timeSavingOn, _weatherSavingOn, _hcObjSavingOn] spawn
	{
		...
		if (_oSave) then
		{
			...
			{
				//call compile preprocessFileLineNumbers "persistence\server\world\oSaveSetup.sqf"; //FIXME: may need to re-enable this, or port it to sock saving
			...
			else
			{
				...
				if ((!_hcObjSavingOn && isServer) || _isHC) then
				{
					...
					//execVM "persistence\server\world\oSave.sqf"; //FIXME: may need to re-enable this, or port it to sock saving
