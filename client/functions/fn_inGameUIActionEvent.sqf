...
if (_unit == player && (_showWindow || _menuOpen)) then
{
	switch (true) do
	{
		...
		case (_action == "UseMagazine"): // placed explosive
		{
			...
			_nearbyMissions = allMapMarkers select {markerType _x == "Empty" && {[["Mission_","ForestMission_","LandConvoy_","SunkenMission_","Sniper_","Roadblock_"], _x] call fn_startsWith && {player distance markerPos _x < _minDist}}};
